...
def print_post_install(paths: list[tuple[str, Path]]) -> str:
    ...
    lines.extend([
        "",
        "Reload this skill in your agent host (new session if required).",
        "",
        "Bootstrap:",
        f"  {cli} init --env",
        "",
        "Agent rules (any AgentSkills host — Cursor, Claude, Hermes, OpenClaw, …):",
        f"  {cli} status   # field: agent_must_know",
        "",
        "MLX + raw CDP (account automation):",
        f"  pip install -e \".[playwright]\" && playwright install chromium",
        f"  {cli} multilogin setup-wizard",
        f"  {cli} connect --account <name>",
        f"  {cli} cdp survey --account <name>",
        f"  {cli} cdp send --account <name> Page.navigate --params '{{\"url\":\"https://...\"}}'",
        f"  {cli} disconnect --account <name>",
        "",
        "Multilogin X:",
